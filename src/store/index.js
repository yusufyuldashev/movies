...
const store = createStore({
  ...
  getters: {
    ...
    moviesGetter(state) {
      return [
        ...new Map(state.movies.map((item) => [item['id'], item])).values(),
      ]
    },
    moviesGetter2(state) {
      return state.movies
  ...
  actions: {
    ...
    async login(context, payload) {
      const response = await fetch(
        'https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=AIzaSyDkiMRrQzEH4Qg8vKhJJiV4TRx1_OmZvGw',
        {
          method: 'POST',
          body: JSON.stringify({
            email: payload.email,
      ...
      if (!response.ok) {
        ...
        const error = new Error(
          responseData.message || 'Failed to authendicate.check again',
        )
        throw error
      }
    ...
    async signup(context, payload) {
      const response = await fetch(
        'https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=AIzaSyDkiMRrQzEH4Qg8vKhJJiV4TRx1_OmZvGw',
        {
          method: 'POST',
          body: JSON.stringify({
            email: payload.email,
      ...
      if (!response.ok) {
        ...
        const error = new Error(
          responseData.message ||
            'This email was used please try with a new one',
        )
        throw error
      }
      localStorage.setItem('token', responseData.idToken)
    ...
    async discoverMovies(context) {
      ...
      const api_url =
        base_url +
        `/discover/movie?sort_by=popularity.desc&${api_key}&language=${this.getters.lang}`
      const response = await fetch(api_url)
      const responseData = await response.json()
      if (!response.ok) {
        const error = new Error(responseData.message || 'Failed to fetch!')
